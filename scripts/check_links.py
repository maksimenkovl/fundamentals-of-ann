#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Проверка Markdown-ссылок и структуры репозитория ФОС.
...
Что проверяется:
  ...
  2. Структура: обязательные файлы; пары kim-NN/rubric-NN в модулях M1, M2,
     Project и Exam; наличие 10 обязательных разделов в каждом КИМ; единая
     10-балльная шкала (10/8/6/4/0-3) в каждой рубрике.
  3. Согласованность БРС: максимум 100 баллов и порог 60 зафиксированы
...
REQUIRED_FILES = [
    ...
    "docs/role-portraits.md",
    "data/competency-catalog.csv",
    "data/role-competency-matrix.csv",
    ...
    "Exam/README.md",
    "Project/README.md",
    "M1-fundamentals-of-ann/README.md",
    "M2-complex-models/README.md",
]

KIM_DIRS = ["M1-fundamentals-of-ann", "M2-complex-models", "Project", "Exam"]

# Обязательные разделы КИМ (подстроки заголовков, см. README § 3).
KIM_SECTIONS = [
    "Назначение",
    "Привязка к КРМ",
    "Цель",
...
BRS_60_FILES = ["README.md", "docs/rpd.md", "Exam/README.md"]
...
def check_structure() -> None:
    ...
    for f in BRS_100_FILES:
        ...
        if p.is_file() and "100" not in p.read_text(encoding="utf-8"):
            err(f"{f}: не зафиксирован максимум БРС 100 баллов")
    for f in BRS_60_FILES:
        p = ROOT / f
        if p.is_file() and "60" not in p.read_text(encoding="utf-8"):
            err(f"{f}: не зафиксирован порог зачёта 60 баллов")
    for md in iter_files("*.md"):
        rel = md.relative_to(ROOT)
